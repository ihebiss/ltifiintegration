    ...
    private void saveUserToPatientTable(User savedUser) {
        {
            for (Role r:savedUser.getRoles()){
             //   if (r.getName()== TypeRole.PATIENT){
               ...
               // }
                if (r.getName() == TypeRole.PATIENT) {
                    String sql = "INSERT INTO patient (user, typatient, archiver, poid, longueur, datedeNais, sexe, nom, prenom, mail) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    jdbcTemplate.update(sql, savedUser.getId(), savedUser.getTypeePatient().toString(), savedUser.getArchiverr(), savedUser.getPoidd(), savedUser.getLongueurr(), savedUser.getDateeDeNaissance(), savedUser.getSexee().toString(), savedUser.getFirstname(),savedUser.getLastname(), savedUser.getEmail());
